...
/**
 * The definitions below correspond to the data model described in the paper. 
 */


export type Context = UnaryContext | MultiaryContext
export type MultiaryContext = Context[];
export interface UnaryContext{source: string, timestamp: string, getSource():string, getTimestamp():string, getUnixTimestamp(): number};


// Data corresponds to an event in the paper's data model. 
...
export interface AggregateData<T> extends Data<T>{
  context: MultiaryContext;
  sequence: number
  getSequence(): number;
};

// Output format of the zipWithTime operator.
export interface Zipped<T>{
  contents: Data<T>[];
  sequence: number
  getSequence(): number;
  getContents(): Data<T>[];
...
export interface Predictor<T>{
  observations: T[];
  addObservation(value:T): void;
  computeEstimate(): T;
}

export interface Combinator<T>{
  combine(value:Zipped<T>): AggregateData<T>;
}

/** OPERATOR INTERFACES */
// extended to include the time context. 

export interface UnaryFunction<T, R> {
  (source: T, context?: any): R;
}
